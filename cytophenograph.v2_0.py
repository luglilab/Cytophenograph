...
parser.add_option('-c', type='choice', choices=['Phenograph', 'Parc', 'Flowsom'],
                  dest="clustering",
                  default = "Phenograph", help='Tool selecting for clustering. Both will execute Phenograph and Parc.')
...
if __name__ == '__main__':
    ...
    if options.clustering == "Phenograph":
        ...
        DictInfo["flowsom_adata"] = run.runflowsom(DictInfo["markertoexclude"], DictInfo["adata_conc"])
        run.groupbycluster(DictInfo["flowsom_adata"], "Flowsom")
        run.groupbysample(DictInfo["flowsom_adata"], "Flowsom")
        run.exporting(DictInfo["flowsom_adata"], options.clustering)
    print("End")
